;;; org2rem.el --- ASCII, HTML, XOXO and iCalendar export for Org-mode
...
(defcustom org-remind-append-passed-remaining-time-warnning t
  "Non-nil means, append warnning about passed and remaining
days to target time, vice-versa. Default is Non-nil."
  :group 'org2rem
  :type 'boolean)
...
(defcustom org-remind-dynamic-priority t
  "Non-nil means, use dynamic prioprity based on remaining days,
vice-versa. Default is Non-nil."
  :group 'org2rem
  :type 'boolean)

(defcustom org-remind-default-dynamic-priority-duration-days 10
  "Days to calculate default prioprity when only deadline or
schedule present."
  :group 'org2rem
  :type 'number)
...
(defcustom org-remind-colored-message t
  "Non-nil include Remind code for colored message."
  :group 'org2rem
  :type 'boolean)
...
When COMBINE is non nil, add the category to each line."
  ...
  (let ((re1 (concat org-ts-regexp "\\|<%%([^>\n]+>"))
	...
	tmp pri categories entry location summary desc uid
        advwarnseconds remind-ew (org-rem-ew org-remind-extra-warn-days)
        (org-rem-advwarndays org-remind-advanced-warn-days)
        trigger diff-seconds (dos org-remind-deadline-over-scheduled)
        (secinday (* 24 60 60))
	...
	(while (re-search-forward org-todo-line-regexp nil t)
	  (catch :skip
	    ...
	    (when (and state
	      ...
	      (setq hd (match-string 3)
		    ...
		    uid (if org-remind-store-UID
              ...
              (if (and due start)
                  (setq diff-seconds (org-rem-time-diff due start)))

              (setq advwarnseconds
                    (if due
                        (if diff-seconds
                            (if (> diff-seconds 0)
                                (if dos diff-seconds 0)
                              (if dos 0 diff-seconds))
                          (* 365 secinday))))
              ...
              (if (and 
                  ...
                  (setq
                   advwarnseconds
                   (+ (or advwarnseconds 0) (* org-rem-advwarndays secinday))))

              (setq remind-ew
                    (if due
                        (if diff-seconds
                            (if (> diff-seconds 0) due nil)
                          due)))

              (setq trigger (if dos (if due due start) (if start start due)))

              (if trigger
                  (let* ((x (org-rem-ts-to-remind-date-type trigger))
                        (trigdate (first x))
                        (trigtime (second x))
                        (advwdays (floor (/ advwarnseconds secinday)))
                        (advwsec  (% (floor advwarnseconds) secinday))
                        (advwmin (if (> advwsec 0) (floor (/ advwsec 60)))))
                   (setq trigger (concat
                                  (format "[trigger('%s')] *%d" trigdate 1)
                                  (if (> advwarnseconds secinday)
                                      (format " ++%d" advwdays))
                                  (if trigtime (concat (format " AT %s" trigtime)
		...
		(setq pri org-default-priority))
	      (setq pri
                    (floor (* 9999 (- 1 (/ (float (- org-lowest-priority pri))
                                           (- org-lowest-priority org-highest-priority))))))

              (setq due   (org-rem-ts-to-remind-date-type due  )
                    start (org-rem-ts-to-remind-date-type start))

              (setq pri
                    (if org-remind-dynamic-priority
                        (let ((default org-remind-default-dynamic-priority-duration-days)
                              (deadline
                               (let ((date (first due))
                                     (time (second due)))
                                 (if date 
                              ...
                              (schedule
                               (let ((date (first start))
                                 ...
                                 (if date
                                     (concat date (concat "@" (or time "00:00")))))))
                          (if (or deadline schedule)
                              (format "[dypriority(%d,%s,%s)]"
                                      pri
                                      (if deadline (format "'%s'" deadline)
                                        (format "'%s' + (%d * 24 * 60)" schedule default))
                                      (if schedule (format "'%s'" schedule)
                                        (format "'%s' - (%d * 24 * 60)" deadline default)))))
                      (format "%d" pri)))

	      (setq hd (org-remind-cleanup-string hd))
...
## END:TODO\n"
				 ...
				 (concat "LOCATION: " location) "")
                             (if (and
                                  org-remind-append-passed-remaining-time-warnning
                                  start)
                                 (concat
                                  "%_\\\n['" (first start) "' - today()] "
...
(defun org-start-remind-file (name)
  ...
  (let ((user user-full-name)
    ...
    (princ
     (concat (format "# -*- Mode: shell-script; auto-fill-mode: nil -*-
## BEGIN: Reminders
## VERSION:2.0
## Emacs with Org-mode
## Calendar:%s
...
## Timezone:%s
## Calscale:Gregorian\n" name user timezone)
             (if org-remind-colored-message
    "## msgprefix definition: start
IF ! defined(\"_initialized_display_attr\")

	SET _initialized_display_attr 1
        # max pripority
        SET mpr 10000
	SET Escape   CHAR(27)

	SET Normal   Escape + \"[0m\"
        # Attribute
        SET Reset      0
        SET Bright     1
        SET Dim        2
        ...
        preserve Escape Normal Reset Bright Dim Underscore Blink Reverse Hidden \
            Black Red Green Yellow Blue Magenta Cyan White mpr Background

# first color, second fore/background, third attribute
# take the help of http://rrbrandt.comli.com/docs/tut/redes/ansi.php
unset jdebug
# array
FSET msgpcolor(color)  choose(color, Red, Magenta, Blue, Green, Cyan, Yellow, Black)
# array
FSET msgpattrb(attrb)  choose(attrb, Reverse, Blink, Underscore, Bright, Reset, Dim)

FSET viscolor(color, attrb)  iif(color == Background && attrb != Reverse, abs(color - 7), color)
FSET create_msgprefix(attrb, color)  iif(defined(\"jdebug\"), \"attrb: \" + attrb + \"color: \" + color \
                                                            + \"[\" + attrb + \";3\" + color +\"m\", \"\") \
                                     + CHAR(27) + \"[\" + msgpattrb(attrb) + \";3\" + viscolor(msgpcolor(color), attrb) + \"m\"

FSET msgprefix(pri)  create_msgprefix( ((pri % (mpr / (7))) / (mpr / (7 * 6))) + 1, \
                                       ((pri % (mpr / (1))) / (mpr / (7)))     + 1 )


# Don't forget to return to normal color set at the end of reminder!
FSET msgsuffix(x) Normal

ENDIF
## msgprefix definition: end
...
(defun org-rem-ts-to-remind-date-type (s)
  (if s (let*
            ((t1 (org-parse-time-string s 'nodefault))
          ...
          (list
           ...
           (if (and (car t1) (nth 1 t1) (nth 2 t1))
               (format-time-string "%H:%M" time)))) nil))

;; (org-rem-ts-to-remind-date-type "<2009-02-22 Sun>")

;; (encode-time
;;  (mapcar (lambda (x) (or x 0)) (org-parse-time-string "<2009-02-22 Sun>" 'nodefault)))
;; (apply 'encode-time (mapcar (lambda (x) (or x 0)) '(0 0 0 22 2 2009 0 0 0)))
...
(defun org-rem-time-diff (end start)
  (apply '- (mapcar
             (lambda (s)
               (let*
                   ((t1 (org-parse-time-string s))
...
;; (org-rem-time-diff "<2009-02-22 Sun 17:00>" "<2009-02-21 Sat 14:30>" )

(provide 'org2rem)

;; arch-tag: 65985fe9-095c-49c7-a7b6-cb4ee15c0a95

;;; org2exp.el ends here
