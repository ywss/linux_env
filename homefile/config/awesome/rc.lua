...
-- Themes define colours, icons, and wallpapers, zenburn
...
for s = 1, screen.count() do
   ...
   -- CHANGE: 默认所有窗口使用tile布局
   tags[s] = awful.tag({ 1, 2, 3, 4, 5, 6, 7, 8, 9,"E","T" }, s, layouts[1])
end
-- }}}
...
for s = 1, screen.count() do
    ...
    mywibox[s].widgets = {
        ...
        mylayoutbox[s],
        -- cpugraph.widget, cpuicon, membar.widget, memicon,
        mytextclock,
...
globalkeys = awful.util.table.join(
    ...
    -- CHANGE: 以下加入自定义的快捷键
    -- CHANGE: Hide / show wibox in awesome 3
    awful.key({ modkey }, "b", function ()
        mywibox[mouse.screen].visible = not mywibox[mouse.screen].visible
    end),
    -- CHANGE: print screen.  好象无效
    ...
    -- CHANGE: 使用Alt+Tab出现应用切换菜单，再单按Alt进行下拉
    awful.key({ altkey }, "Tab", function ()
                                  -- If you want to always position the menu on the same place
                                  --mouse.coords({x=525, y=330}, true)
                                  awful.menu.menu_keys.up = { "p" }
                                  awful.menu.menu_keys.down = { "Down", "Alt_L", modkey, "j", "n" }
    ...
    -- CHANGE: 更好的dmenu
    awful.key({modkey }, "p", function() awful.util.spawn( "smart-dmenu" ) end),
...
clientkeys = awful.util.table.join(
    ...
    -- 自动打开
    awful.key({ modkey, }, "g", function () run_or_raise("google-chrome", { name = "Google Chrome" }) end),
    awful.key({ modkey, }, "t", function () run_or_raise("start-tmux", { name = "tmux-main" }) end),
    awful.key({ modkey, }, "o", function () run_or_raise("opera", { class = "Opera" }) end),
    awful.key({ modkey, }, "w", function () run_or_raise("vwin2003", { class = "VirtualBox", name = "Win2003" }) end)
)
...
awful.rules.rules = {
    ...
    { rule = { class = "Opera" },
      properties = { tag = tags[secound_screen][7], switchtotag = true, maximized_vertical = true, maximized_horizontal = true } },

    { rule = { class = "VirtualBox" },
      properties = { tag = tags[secound_screen][5], switchtotag = true, floating = true } },
...
-- Autostart
-- via http://awesome.naquadah.org/wiki/Autostart#Directory_way
-- for j in ${(f)"$(for i in ~/.config/autostart/*.desktop; do grep '^Exec=' $i | sed 's/^Exec=//'; done)"}; do $j &| ; done
function autostart(dir)
    if not dir then
        do return nil end
    end
    local fd = io.popen("/bin/ls -1 -F " .. dir)
    if not fd then
        do return nil end
    end
